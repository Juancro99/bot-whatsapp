...
Esta funcion se utliza para ejecutar una acción despues del `addKeyword()`  
A diferencia de `.addAnwser()`, no cuenta con opciones, ni puede anidar flujos.   

```js  
// Este ejemplo responde "Hola ¿cómo estas?" usando baileys
// 
const flujoBienvenida = addKeyword('hola')
    .addAction(
        async(ctx, {provider}) => {  
            const id = ctx.key.remoteJi  
            const message = "Hola, ¿Cómo estas?"
            const a = await provider.getInstance()      
            await a.sendMessage(id, message)  
